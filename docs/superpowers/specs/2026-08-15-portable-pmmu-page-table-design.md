...
Add `CONF_WITH_MMU_MAINT` under `Kconfig.machine`, defaulting to enabled where a backend exists. For this issue: `CONF_WITH_MMU_MAINT` is `y` when `CONF_WITH_ARM_PMMU && (MACHINE_RPI || MACHINE_VIRT_ARM) && !CONF_WITH_ARM_LPAE`.

The `!CONF_WITH_ARM_LPAE` clause is required, not defensive boilerplate: `CONF_WITH_ARM_LPAE` (added by issue #276's RPi4 PCIe work, `bios/machine/raspi/raspi_lpae.c`) already exists in this tree, depends on `TARGET_RPI4 && CONF_WITH_ARM_PMMU && !CONF_WITH_MMU_TEXT_PROTECT`, and `TARGET_RPI4` satisfies `MACHINE_RPI`. Without this exclusion, a RPi4 build could enable both this short-descriptor maintenance layer and the LPAE long-descriptor bootstrap at once; they use incompatible table formats, and the generic walker has no way to detect which one actually initialized the hardware. This mirrors the exclusivity `CONF_WITH_ARM_LPAE` itself already has against `CONF_WITH_MMU_TEXT_PROTECT`, for the same reason.
...
- **Phase 3 (LPAE/highmem, issue #76):** LPAE backend with 64-bit `phys_addr_type`, dynamic device mappings, and `ioremap()`/`iounmap()`; move `pci_phys_to_virt()` and PCI resource mapping onto `mmu_map_range()`. This phase is not starting from nothing: issue #276's RPi4 PCIe work already landed a narrow, opt-in, hardware-validated LPAE bootstrap (`CONF_WITH_ARM_LPAE`, `bios/machine/raspi/raspi_lpae.c`) that maps a fixed 64 MiB device window (`0xf8000000`-`0xfbffffff` virtual to `0x6_00000000`-`0x603fffff` physical) for the BCM2711 PCIe aperture on real Pi 400, using ARMv7-A long descriptors with one 4-entry, table-only L1 and four 512-entry L2 tables of 2 MiB block leaves (no L3/4 KiB level; that stopgap needs no page granularity). Phase 3's job is to bring that hand-built, static tree under this design's generic walker and table registry — the same relationship Phase 1 has to `virt_mmu_bootstrap()`/`init_mmu()` for the short-descriptor backend — rather than reinvent LPAE bring-up from scratch; the RPi4 bootstrap sequence (`MAIR`/`TTBCR.EAE`/64-bit `TTBR` load via `mcrr`, TLB invalidate, DSB/ISB ordering) is a validated reference for the new backend's own bootstrap-handoff step. Note issue #76's own body sketches `phys_addr_t`/`bus_addr_t`/`vaddr_t`, predating this repo's `_type` naming convention (see `CLAUDE.md`); Phase 3 uses `phys_addr_type`/`bus_addr_type`/`virt_addr_type` instead. `bios/machine/raspi/raspi_pci.c`'s `raspi_pci_bus_to_phys()` is a concrete, already-existing instance of the address-model conflation this design exists to fix: under `CONF_WITH_ARM_LPAE` it is named `_phys` but deliberately returns the *virtual* aperture address (`0xf8000000`, not the true `0x6_00000000` physical address), because nothing downstream can consume a real 64-bit physical address yet — its own design doc says as much ("The eventual #76 API cleanup must distinguish bus, physical, and virtual addresses explicitly"). Phase 3 should convert this call, and the rest of `pci_core.c`'s `ULONG`-typed resource fields, onto explicit `phys_addr_type`/`bus_addr_type`/`virt_addr_type`.
...
- `CONF_WITH_ARM_LPAE` (issue #276, `bios/machine/raspi/raspi_lpae.c`) already exists on `TARGET_RPI4` and uses a page-table format this Phase 1 layer's ARM short-descriptor backend cannot walk; Configuration And Build's `!CONF_WITH_ARM_LPAE` clause on `CONF_WITH_MMU_MAINT` is required to keep the two from being enabled together on the same target, not just tidy Kconfig hygiene.
