...
- Follow-up bug found in the first desktop screendump (reported by the user): the menubar read `!Edrj!!Ghmd!!Whdv!!Nquhnor!` instead of `Desk  File  View  Options`. Root cause: `truecolor_text_blit()` read its source through native `UWORD *p` loads, but the text source (font, or `pre_blit()`'s buffer) is a big-endian Motorola byte stream; on little-endian ARM each adjacent pair of glyph bytes is swapped, so even char codes render their successor and odd ones their predecessor. The earlier T2S8 pass signal (any non-background pixels in the strip) was too weak to catch this. Fixed in `97c1b624` by reading each source word as `(p[0] << 8) | p[1]` (matching `normal_blit()` in vdi/arch/arm/vdi_tblit.c), in all four WRT_MODE loops. Verified by decoding the screendump menu strip against the 8x16 font: 14/14 cells match `Desk  File  Vi` at cell origin x0=24,y0=1, full strip decodes to `Desk  File  View  Options`. Rebuilt rpi1/virt-arm/virt-m68k; rpi1 and virt-arm boot to `evnt_multi()` with the fixed text.
