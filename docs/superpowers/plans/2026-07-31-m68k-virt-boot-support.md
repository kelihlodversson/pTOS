...
```
Expected: the process runs for the full 5 seconds (killed by `timeout`, exit code 124), and `virt-m68k-serial.log` is empty (no console driver yet). `virt-m68k-qemu.log` may contain **one** benign `Illegal Instruction` entry: `bios/arch/m68k/processor.S`'s `_detect_fpu` (called right after `_detect_cpu` from `processor_init()`) deliberately executes an `FNOP` to probe for an FPU, protected by its own temporary Line-F exception vector — on `-cpu m68020` with no FPU model attached, QEMU has no FPU to answer, so the CPU takes a Line-F exception, which `_detect_fpu` catches and correctly reports "no FPU" (not the CACR/PCR probe in `_detect_cpu` described in "Additional gaps..." above — that one resolves to "68020" without ever trapping, which is *why* `-cpu m68020` avoids the fatal QEMU bug in the first place; this is a second, separate, always-expected trap from the very next detection step). That specific, single, early entry is expected and means detection completed correctly, not a crash. What would indicate a real problem: any entry *after* that one, more than one entry, an "unimplemented device" entry, or the process exiting before the timeout fires — any of those mean something in the boot chain (bss clear, `_phystop`, stack) is wrong.
