...
No config.txt is needed: the "-32" in kernel8-32.img is enough on its own
to force 32-bit mode on the Pi 3, 3+ and CM3, which can otherwise also run
64-bit code.

Write ptos-raspi-<version>.img (after unzipping it) directly to an SD card
with dd, Raspberry Pi Imager or balenaEtcher. Alternatively, on a card you
have already partitioned and formatted yourself, just copy over the
contents of ptos-raspi-<version>.zip.
