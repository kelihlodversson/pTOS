...
reverse_linefeed (void)
{
    ...
    if ( linea_vars.v_cur_cy ) {
        ...
        int savex = linea_vars.v_cur_cx;           /* save current x position */
...
cursor_off (void)
{
    ...
    linea_vars.v_stat_0 &= ~M_CVIS;                /* make invisible! */
    ...
    else if ( linea_vars.v_stat_0 & M_CSTATE ) {
        linea_vars.v_stat_0 &= ~M_CSTATE;    /* cursor off? */
        invert_cell(linea_vars.v_cur_cx, linea_vars.v_cur_cy);
    }
}
...
cursconf(WORD function, WORD operand)
{
    ...
    case 1:
        ...
        break;
    case 2:
        linea_vars.v_stat_0 &= ~M_CFLASH;          /* unset cursor flash bit */
        break;
    case 3:
...
vt52_init(void)
{
    ...
    linea_vars.v_cur_of = 0;            /* line offset is 0 */
    linea_vars.v_cur_ad = v_bas_ad;     /* set cursor to start of screen */

    linea_vars.v_stat_0 = M_CFLASH;     /* cursor invisible, flash, nowrap, normal video */
    cursconf(4, 30);                    /* 0.5 second blink rate (@ 60Hz vblank) */
    linea_vars.v_cur_tim = linea_vars.v_period;               /* load initial value to blink timer */
    linea_vars.disab_cnt = 1;           /* cursor disabled 1 level deep */

    /* set foreground color depending on color depth */
    switch (linea_vars.v_planes) {
    case 1:
        linea_vars.v_col_fg = 1;
        break;
    case 2:
        linea_vars.v_col_fg = 3;
        break;
    default:
        linea_vars.v_col_fg = 15;
        break;
    }
    linea_vars.v_col_bg = 0;
