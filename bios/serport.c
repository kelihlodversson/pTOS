 ...
 *
 * CONF_WITH_COLDFIRE_RS232 and CONF_WITH_MFP_RS232 route input through
 * the interrupt-fed iorec1 (mfp_rs232_rx_interrupt_handler / the ColdFire
 * UART interrupt both call push_serial_iorec()), matching the SCC/TT-MFP
 * ports below -- reading the hardware registers directly here would race
 * the interrupt handler that already drains them.
 */
LONG bconstat1(void)
{
#if CONF_WITH_RASPI_UART0
