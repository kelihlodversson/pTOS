...
{
#if !CONF_ATARI_HARDWARE && defined(__m68k__)
     ...
     * GSX_ENTRY. It predates vditrap()/vdistub.h and was never converted
     * to that newer interface, so it isn't declared there -- without this
     * assignment, VEC_GEM keeps its any_vec() panic-on-use default and
