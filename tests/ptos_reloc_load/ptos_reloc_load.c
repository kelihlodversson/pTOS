 ...
 *
 * Launches reloc_probe.c, a separate executable built as a fixed-base
 * ET_EXEC ("ld -q", --emit-relocs) and then repacked by
 * tools/ptos-elf-pack.c into the compact .ptos.reloc / PT_PTOS_RELOC
 * format (doc/elfload.txt), via Pexec() and checks its exit code.
 ...
 * path #309 added, which the existing pie_load suite's ET_DYN payload
 * does not exercise.
 *
