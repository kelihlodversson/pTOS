...
static void hwblit_vertical_line(const Line *line, WORD wrt_mode, UWORD color)
{
    ...
    dy = line->y2 - line->y1;
    yinc = linea_vars.v_lin_wr;

    if (dy >= 0)
    {
        for (i = 0, mask = 0x8000; i < 16; i++, mask >>= 1)
            BLITTER->halftone[i] = (linea_vars.LN_MASK & mask) ? 0xffff : 0x0000;
        start_line = 0;
    }
    else
    {
    ...
    height = dy + 1;
    size = muls(height, linea_vars.v_lin_wr);
    ...
    /* update linea_vars.LN_MASK for next time */
    mask = linea_vars.LN_MASK;
    for (i = height & 0x000f; i; i--)
        rolw1(mask);
    linea_vars.LN_MASK = mask;
}
#endif
...
static void hwblit_rect_nonstd(const VwkAttrib *attr, const Rect *rect, UWORD *addr)
{
     ...
     * invalidate any cached screen data
     */
    invalidate_data_cache(addr, linea_vars.v_lin_wr*ycount);
}
...
{
    const UWORD patmsk = attr->patmsk;
    const int yinc = linea_vars.v_lin_wr >> 1;     /* in WORDs */
...
{
    const UWORD patmsk = attr->patmsk;
    const int vplanes = linea_vars.v_planes;
    const int yinc = (linea_vars.v_lin_wr>>1) - vplanes;
    int centre, y;
    BLITPARM b;
...
void linea_rect(void)
{
    ...
    if (linea_vars.CLIP) {
        if (linea_vars.X1 < linea_vars.XMINCL)
            linea_vars.X1 = linea_vars.XMINCL;
        if (linea_vars.X2 > linea_vars.XMAXCL)
            linea_vars.X2 = linea_vars.XMAXCL;
        if (linea_vars.Y1 < linea_vars.YMINCL)
            linea_vars.Y1 = linea_vars.YMINCL;
        if (linea_vars.Y2 > linea_vars.YMAXCL)
            linea_vars.Y2 = linea_vars.YMAXCL;
    }
...
void linea_polygon(void)
{
    ...
    clc_flit(&attr, &clipper, points, count, linea_vars.Y1, linea_vars.Y1-1);
}
...
void polyline(Vwk * vwk, Point * point, int count, WORD color)
{
    ...
    for (i = count-1, linea_vars.LSTLIN = FALSE; i > 0; i--) {
        if (i == 1)
            linea_vars.LSTLIN = TRUE;
        line.x1 = point->x;
        line.y1 = point->y;
...
static void draw_line16(const Line *line, WORD wrt_mode, UWORD color)
{
    ...
    dx = line->x2 - line->x1;
    dy = line->y2 - line->y1;
    yinc = linea_vars.v_lin_wr / 2;        /* in words */
 ...
 * Bresenham's algorithm.  The line is modified by the linea_vars.LN_MASK
 ...
 * linea_vars.LN_MASK line style and wrt_mode write mode. abline() above handles
 ...
 *
 * returns: linea_vars.LN_MASK rotated to proper alignment with (x2,y2).
 */
UWORD
...
static void swblit_vertical_line16(const Line *line, WORD wrt_mode, UWORD color)
{
    ...
    dy = line->y2 - line->y1;
    yinc = linea_vars.v_lin_wr / 2;        /* one line of words */
...
static void vertical_line(const Line *line, WORD wrt_mode, UWORD color)
{
    ...
    UWORD bit, bitcomp;
    WORD plane, loopcnt;
    UWORD linemask = linea_vars.LN_MASK;
    ...
    for (plane = linea_vars.v_planes-1; plane >= 0; plane--, start++, color >>= 1) {
        /* load values fresh for this bitplane */
 ...
 *     linea_vars.v_planes     = number of video planes
 *     linea_vars.LN_MASK      = line mask (for dashed/dotted lines)
 ...
 *
 * output:
 *     linea_vars.LN_MASK rotated to proper alignment with x coordinate of line end
 */
void abline(const Line *line, const WORD wrt_mode, UWORD color)
{
     ...
     */
    if ((wrt_mode == WM_XOR) && !linea_vars.LSTLIN)
    {
        if (x1 != x2)
            x2--;
        else if (y1 != y2)
...
void linea_line(void)
{
    ...
    /* Line-A linea_vars.LN_MASK is already set by caller */
    abline(&line, linea_vars.WRT_MODE, linea_color());
}


/*
 * set linea_vars.LN_MASK from virtual workstation values
 */
