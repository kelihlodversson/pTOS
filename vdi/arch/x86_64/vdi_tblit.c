 ...
 *
 * Identical to vdi/arch/arm/vdi_tblit.c: this is already arch-neutral C
 * (only asm.h's rolw1() macro, which this arch implements for real --
 * unlike the deliberately-broken set_sr()/swpw() placeholders -- see
 * that file's own comment). Kept as a separate per-arch copy rather than
 * a generic location for the same reason as vdi_entry.c in this same
 * directory.
 *
