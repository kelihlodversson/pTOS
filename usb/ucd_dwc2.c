...
static int submit_bulk_msg(struct usb_device *dev, unsigned long pipe, void *buffer, int len);
...
static long dwc2_open(struct ucdif *u);
static long dwc2_close(struct ucdif *u);
static long dwc2_ioctl(struct ucdif *u, short cmd, long arg);
...
static struct dwc2_priv dwc2_local;
static struct ucdif dwc2_uif =
{
    0,                  /* *next */
    USB_API_VERSION,    /* API */
    USB_CONTRLL,        /* class */
...
static void init_fslspclksel(struct dwc2_core_regs *regs)
{
    ...
    uint32_t ghwcfg2 = readl(&regs->ghwcfg2);
...
static void dwc_otg_flush_tx_fifo(struct dwc2_core_regs *regs, const int num)
{
    ...
    writel(DWC2_GRSTCTL_TXFFLSH | (num << DWC2_GRSTCTL_TXFNUM_OFFSET),
           &regs->grstctl);
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_TXFFLSH, FALSE, 1000);
    if (ret)
        KINFO(("%s: Timeout!\n", __func__));
...
static void dwc_otg_flush_rx_fifo(struct dwc2_core_regs *regs)
{
    ...
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_RXFFLSH, FALSE, 1000);
    if (ret)
        KINFO(("%s: Timeout!\n", __func__));
...
static void dwc_otg_core_reset(struct dwc2_core_regs *regs)
{
    ...
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_AHBIDLE, TRUE, 1000);
    if (ret)
        KINFO(("%s: Timeout!\n", __func__));

    /* Core Soft Reset */
    writel(DWC2_GRSTCTL_CSFTRST, &regs->grstctl);
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_CSFTRST, FALSE, 1000);
    if (ret)
        KINFO(("%s: Timeout!\n", __func__));
...
static void dwc_otg_core_host_init(struct dwc2_core_regs *regs)
{
    ...
    for (i = 0; i < num_channels; i++) {
        ...
        ret = wait_for_bit_le32(&regs->hc_regs[i].hcchar, DWC2_HCCHAR_CHEN, FALSE, 1000);
        if (ret)
            KINFO(("%s: Timeout!\n", __func__));
    }
                       ...
                       int txlen, struct devrequest *cmd)
{
    ...
    case 0:
        *(uint16_t *)buffer = cpu2le16(1);
        len = 2;
        break;
    case USB_RECIP_INTERFACE:
    case USB_RECIP_ENDPOINT:
        *(uint16_t *)buffer = cpu2le16(0);
        len = 2;
        break;
    case USB_TYPE_CLASS:
        *(uint32_t *)buffer = cpu2le32(0);
        len = 4;
        break;
    case USB_RECIP_OTHER | USB_TYPE_CLASS:
        ...
        *(uint32_t *)buffer = cpu2le32(port_status |
                    (port_change << 16));
        len = 4;
        break;
    default:
        KINFO(("unsupported root hub command\n"));
        stat = USB_ST_STALLED;
    }
                           ...
                           struct devrequest *cmd)
{
    ...
    int len = 0;
    int stat = 0;
    uint16_t wValue = cpu2le16(cmd->value);
    uint16_t wLength = cpu2le16(cmd->length);
                 ...
                 struct devrequest *cmd)
{
    ...
    if (usb_pipeint(pipe)) {
        KINFO(("Root-Hub submit IRQ: NOT implemented\n"));
        return 0;
    }
...
int wait_for_chhltd(struct dwc2_hc_regs *hc_regs, uint32_t *sub, uint8_t *toggle)
{
    ...
    ret = wait_for_bit_le32(&hc_regs->hcint, DWC2_HCINT_CHHLTD, TRUE, 2000);
    if (ret)
        return ret;
    ...
    *toggle = (hctsiz & DWC2_HCTSIZ_PID_MASK) >> DWC2_HCTSIZ_PID_OFFSET;
    ...
    KDEBUG(("%s: HCINT=%08x sub=%u toggle=%d\n", __func__, hcint, *sub,
          *toggle));
              ...
              int xfer_len, int *actual_len, int odd_frame)
{
    ...
    KDEBUG(("%s: chunk: pid %d xfer_len %u pkts %u\n", __func__,
    ...
    if (xfer_len) {
        if (in) {
            memset(aligned_buffer,'a',xfer_len);
    ...
    if (in) {
        ...
        invalidate_data_cache(aligned_buffer,
                    roundup(xfer_len, ARCH_DMA_MINALIGN));
          ...
          unsigned long pipe, uint8_t *pid, int in, void *buffer, int len)
{
    ...
    KDEBUG(("%s: msg: pipe %lx pid %d in %d len %d\n", __func__, pipe, *pid,
          in, len));
...
static int submit_bulk_msg(struct usb_device *dev, unsigned long pipe, void *buffer, int len)
{
    struct dwc2_priv *priv = (struct dwc2_priv *)dev->controller->ucd_priv;
                           ...
                           void *buffer, int len, int interval)
{
    ...
    for (;;) {
        if (get_timer(0) > timeout) {
            KINFO(("Timeout poll on interrupt endpoint\n"));
            return ETIMEDOUT;
        }
...
static int dwc2_init_core(struct dwc2_priv *priv)
{
    ...
    KINFO(("Core Release: %lx.%03lx\n",
         snpsid >> 12 & 0xf, snpsid & 0xfff));

    if ((snpsid & DWC2_SNPSID_DEVID_MASK) != DWC2_SNPSID_DEVID_VER_2xx &&
        (snpsid & DWC2_SNPSID_DEVID_MASK) != DWC2_SNPSID_DEVID_VER_3xx) {
        KINFO(("SNPSID invalid (not DWC2 OTG device): %08lx\n",
             snpsid));
        return EUNDEV;
    }
...
static long dwc2_power_on(void)
{
    ...
    {
        KINFO(("DWC HCD sevice did not power on successfully\n"));
        return -1;
    }
...
{
    struct dwc2_core_regs * regs = (struct dwc2_core_regs *)USB_DWC2_REG_ADDR;
    ...
    clrbits_le32(&regs->gahbcfg, DWC2_GAHBCFG_GLBLINTRMSK);
...
void dwc2_init(void)
{
    if (ucd_register(&dwc2_uif, &root_hub_dev))
    {
        KINFO(("dwc2_init(): ucd register failed!"));
        return;
    }
    KDEBUG(("dwc2_init(): ucd register succeeded!"));
}
