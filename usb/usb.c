            ...
            void *data, unsigned short size, long timeout)
{
    ...
    KDEBUG(("usb_control_msg: request: 0x%x, requesttype: 0x%x, value 0x%x idx 0x%x length 0x%x\n",
...
usb_set_maxpacket_ep(struct usb_device *dev, int if_idx, int ep_idx)
{
    ...
    if ((ep->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) ==
        ...
        KDEBUG(("##Control EP epmaxpacketout/in[%d] = %ld\n",
...
long usb_parse_config(struct usb_device *dev, unsigned char *buffer, long cfgno)
{
    ...
    if (head->bDescriptorType != USB_DT_CONFIG) {
        KDEBUG((" ERROR: NOT USB_CONFIG_DESC %x\n",
    ...
    while (index + 1 < dev->config.desc.wTotalLength && head->bLength) {
        ...
        default:
            ...
            KDEBUG(("unknown Description Type : %x\n", head->bDescriptorType));
            break;
        }
        index += head->bLength;
        head = (struct usb_descriptor_header *)&buffer[index];
...
long usb_set_interface(struct usb_device *dev, long interface, long alternate)
{
    ...
    if (!if_face) {
        KDEBUG(("selecting invalid interface %ld\n", interface));
        return -1;
    }
    /*
...
long usb_string(struct usb_device *dev, long index, char *buf, long size)
{
    ...
    if (!dev->have_langid) {
        ...
        if (err < 0) {
            KDEBUG(("error getting string descriptor 0 \n" \
                   "(error=%lx)", dev->status));
            goto errout;
        } else if (tbuf[0] < 4) {
            KDEBUG(("string descriptor 0 too short\n"));
            err = -1;
            goto errout;
        } else {
            dev->have_langid = -1;
            ...
            KDEBUG(("USB device number %ld default \n" \
                   "language ID 0x%lx",
...
usb_find_interface_driver(struct usb_device *dev, unsigned ifnum)
{
    ...
    if ((!dev) || (ifnum >= dev->config.desc.bNumInterfaces)) {
        KDEBUG(("bad find_interface_driver params\n"));
        return -1;
    }
    ...
    while (driver)
    {
        if (!driver->probe(dev, ifnum))
        {
            ...
            KDEBUG(("driver attached to iface %d\n", ifnum));
            return 0;
        }
