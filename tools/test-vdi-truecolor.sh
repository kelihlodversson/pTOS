...
# truecolor op-table reference compiled in, exercising the real backend.
mkdir -p "$tmpdir/inc"
printf '#define CONF_WITH_VDI_TRUECOLOR 1\n' > "$tmpdir/inc/autoconf.h"

${CC:-cc} -std=gnu90 -Wall -Wextra -Werror \
    -I"$tmpdir/inc" -Iinclude -Ivdi \
    tools/test-vdi-truecolor.c vdi/vdi_backend.c \
    vdi/vdi_backend_truecolor.c bios/screen_mode.c \
    -o "$tmpdir/test-vdi-truecolor"
"$tmpdir/test-vdi-truecolor"
