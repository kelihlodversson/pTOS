...
# ptos_abi_import launches a native ELF payload (abi_probe.c, built
# below) linked against the SDK's libptos-abi.so.1 stub and packed with
# tools/ptos-elf-pack.c, to exercise bdos/elfld.c's native pTOS ABI
...
# CONF_WITH_PTOS_ABI_IMPORTS specifically, not just CONF_WITH_ELF_LOADER:
# without it there is no kernel export table for the packed imports to
# resolve against, and Pexec() would refuse to load the payload outright
# rather than exercise anything.
#
# libptos-abi.so.1 is never loaded by pTOS, at build time or at runtime
# (doc/elfload.txt): it exists only so the linker below produces real
# ELF dynamic-import machinery (.dynsym/.rela.plt/.got.plt) for every
# "gemdos:" symbol abi_probe.c calls, which ptos-elf-pack then converts.
LIBPTOSABI_SONAME = libptos-abi.so.1

# -mshort on m68k (never on ARM, which has no such flag and whose native
# int is already 32 bits, matching the kernel): the kernel's GEMDOS
# handlers overwhelmingly declare their WORD-sized parameters as plain
# "int", relying on -mshort to make that 16 bits -- and critically,
# -mshort also narrows the *stack argument width* m68k GCC generates for
# such a parameter (2 bytes instead of 4), independently of the callee's
# declared type. TEST_CFLAGS deliberately omits -mshort (see its own
# comment above: ordinary userland code must not inherit kernel-only
# conventions), so without this override here specifically, a call from
# this pair of files into any real kernel GEMDOS handler pushes every
# argument after the first WORD-sized one at the wrong stack offset --
# reproduced directly: an unpatched build's Fwrite(handle, len, buf) call
# reached the kernel's xwrite() as xwrite(0, 393216, <garbage>) instead of
# the real (6, 34, <buffer>), because a 32-bit stack push for "handle"
# shifted every later argument by 2 bytes past where -mshort-compiled
...
obj/stub_gemdos.o: tests/ptos_abi_import/stub_gemdos.c $(AUTOCONF_H) | obj
	$(CC) $(TEST_CFLAGS) $(PTOSABI_CFLAGS) -Iinclude $(DEPFLAGS) -c $< -o $@

obj/abi_probe.o: tests/ptos_abi_import/abi_probe.c $(AUTOCONF_H) | obj
	$(CC) $(TEST_CFLAGS) $(PTOSABI_CFLAGS) -Iinclude $(DEPFLAGS) -c $< -o $@

ifdef ARCH_ARM
$(LIBPTOSABI_SONAME): obj/stub_gemdos.o
	$(CC) $(CPUFLAGS) -nostartfiles -nostdlib -shared \
	    -Wl,-soname,$(LIBPTOSABI_SONAME) $< -o $@

abiprobe-unpacked.tos: $(TEST_STARTUP) obj/abi_probe.o $(LIBCMINI_LIB) $(LIBPTOSABI_SONAME)
	$(TEST_LD) -Wl,-pie -Wl,--no-dynamic-linker -Wl,-e_start \
	    $(TEST_STARTUP) obj/abi_probe.o -L$(dir $(LIBCMINI_LIB)) -lcmini \
	    -L. -l:$(LIBPTOSABI_SONAME) $(LIBS) -o $@
else
# m68k-atari-mintelf-ld's default, driver-selected emulation refuses both
# -shared and -pie outright ("-pie not supported", the same pre-existing
# limitation TEST_PTOS_RELOC_LDFLAGS's own comment above documents for
# relocprobe-unpacked.tos). Its bare "m68kelf" emulation, invoked
# directly rather than through the $(CC) driver, accepts both --
# verified directly with this toolchain: a real libptos-abi.so.1 built
# this way, and abi_probe.o linked against it the same way, produces
# genuine R_68K_JMP_SLOT (.rela.plt) and R_68K_32-against-undefined-
# symbol (.rela.dyn -- this toolchain has no -fPIC/-fPIE code
# generation, so a call to an imported function compiles to a plain
# absolute reference rather than a PLT-relative one) relocations for
# every imported name, confirmed with readelf.
$(LIBPTOSABI_SONAME): obj/stub_gemdos.o
	$(CROSS_COMPILE)ld -m m68kelf --oformat=elf32-m68k -shared \
	    -soname $(LIBPTOSABI_SONAME) $< -o $@

abiprobe-unpacked.tos: $(TEST_STARTUP) obj/abi_probe.o $(LIBCMINI_LIB) $(LIBPTOSABI_SONAME)
	$(CROSS_COMPILE)ld -m m68kelf --oformat=elf32-m68k -pie \
	    --no-dynamic-linker -e _start \
	    $(TEST_STARTUP) obj/abi_probe.o -L$(dir $(LIBCMINI_LIB)) -lcmini \
	    -L. -l:$(LIBPTOSABI_SONAME) -L$(TEST_LIBGCC_DIR) -lgcc -o $@
endif

ABIPROBE.TOS: abiprobe-unpacked.tos ptos-elf-pack
...
TEST_HD_FILES = runtests.tos tests/emudesk.inf $(TEST_PIE_FILES) $(TEST_PTOS_RELOC_FILES) $(TEST_PTOS_ABI_FILES)

test-hd.img: runtests.tos tests/emudesk.inf $(TEST_PIE_FILES) $(TEST_PTOS_RELOC_FILES) $(TEST_PTOS_ABI_FILES) $(shell find $(TEST_DESTDIR) -type f)
	@echo '  MKHD   $@'
...
TOCLEAN += tests/run_tests.c runtests.tos pieprobe.tos \
           ...
           libptos-abi.so.1 abiprobe-unpacked.tos ABIPROBE.TOS \
           test-hd.img
