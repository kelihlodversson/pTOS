...
static void just_draw(OBJECT *tree, WORD obj, WORD sx, WORD sy)
{
    ...
    {
        movetext = FALSE;
        changecol = !xor_is_ok(obtype, flags, spec.index);
    }
#endif
        ...
        (obtype == G_BUTTON))
    {
        ...
        if (len)
        {
             ...
             */
            if ((obtype == G_STRING) && (state & DISABLED) && (len > 3) && is_dashes(spec.free_string, len))
            {
...
void ob_change(OBJECT *tree, WORD obj, UWORD new_state, WORD redraw)
{
    ...
    default:        /* others: handle change of SELECTED state */
        ...
        {
            BOOL ok = xor_is_ok(obtype,flags, spec.index);
